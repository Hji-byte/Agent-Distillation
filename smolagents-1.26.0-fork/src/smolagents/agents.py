...
class CodeAgent(MultiStepAgent):
    ...
    Args:
        ...
        required_action_prefix (`str`, *optional*): Require each generated action to contain a non-empty line beginning with this prefix.
        max_action_format_retries (`int`, default `0`): Number of immediate retries for an action that omits `required_action_prefix`.
        **kwargs: Additional keyword arguments.
    """
    ...
    def __init__(
        ...
        required_action_prefix: str | None = None,
        max_action_format_retries: int = 0,
        **kwargs,
    ):
        self.additional_authorized_imports = additional_authorized_imports if additional_authorized_imports else []
        self.authorized_imports = sorted(set(BASE_BUILTIN_MODULES) | set(self.additional_authorized_imports))
        self.max_print_outputs_length = max_print_outputs_length
        if max_action_format_retries < 0:
            raise ValueError("`max_action_format_retries` must be non-negative.")
        if max_action_format_retries and not required_action_prefix:
            raise ValueError("`required_action_prefix` is required when action format retries are enabled.")
        if stream_outputs and max_action_format_retries:
            raise ValueError("Action format retries are not supported with `stream_outputs=True`.")
        self.required_action_prefix = required_action_prefix
        self.max_action_format_retries = max_action_format_retries
    ...
    def _has_required_action_prefix(self, output_text: str | None) -> bool:
        if not self.required_action_prefix:
            return True
        pattern = rf"(?m)^[ \t]*{re.escape(self.required_action_prefix)}[ \t]*\S"
        return bool(output_text and re.search(pattern, output_text))

    def _action_format_retry_messages(
        self, input_messages: list[ChatMessage], invalid_message: ChatMessage
    ) -> list[ChatMessage]:
        """Build a temporary correction exchange without adding it to agent memory."""
        retry_messages = input_messages.copy()
        retry_messages.append(invalid_message)
                ...
                content=[
                    {
                        ...
                        "text": (
                            ...
                            f"`{self.required_action_prefix}`. Regenerate only the current assistant action. "
                            f"Start with `{self.required_action_prefix} <reasoning>`, then provide the complete "
                            f"{self.code_block_tags[0]}...{self.code_block_tags[1]} action. "
                            "Do not discuss the correction."
                        ),
                    }
                ],
            )
    ...
    def _step_stream(
        ...
        try:
            ...
            if self.stream_outputs:
                ...
                consumed_input_tokens = chat_message.token_usage.input_tokens if chat_message.token_usage else 0
                consumed_output_tokens = chat_message.token_usage.output_tokens if chat_message.token_usage else 0
                has_token_usage = chat_message.token_usage is not None
                while (
                    not self._has_required_action_prefix(chat_message.content)
                    and memory_step.format_retry_count < self.max_action_format_retries
                ):
                    memory_step.format_retry_count += 1
                    retry_messages = self._action_format_retry_messages(input_messages, chat_message)
                    ...
                    if chat_message.token_usage:
                        ...
                        consumed_input_tokens += chat_message.token_usage.input_tokens
                        consumed_output_tokens += chat_message.token_usage.output_tokens

                memory_step.model_output_message = chat_message
                output_text = chat_message.content
                if memory_step.format_retry_count and has_token_usage:
            ...
            if memory_step.token_usage is None:
                memory_step.token_usage = chat_message.token_usage
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        agent_dict["required_action_prefix"] = self.required_action_prefix
        agent_dict["max_action_format_retries"] = self.max_action_format_retries
        return agent_dict
    ...
    def from_dict(cls, agent_dict: dict[str, Any], **kwargs) -> "CodeAgent":
        ...
        code_agent_kwargs = {
            ...
            "required_action_prefix": agent_dict.get("required_action_prefix"),
            "max_action_format_retries": agent_dict.get("max_action_format_retries"),
        }
