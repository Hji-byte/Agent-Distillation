...
model="${1:?Usage: bash scripts/training/run_cot_sft_6400.sh MODEL normal|shortest [--preflight|--smoke|--resume]}"
...
"$python_bin" -u -m scripts.training.train_cot_pair \
  ...
  --output "training_outputs/Qwen3.5-0.8B/cot_${variant}_max6400_qlora_seed42${suffix}" "$@"
