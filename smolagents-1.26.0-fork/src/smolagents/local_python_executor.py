...
def timeout(timeout_seconds: int):
    ...
    On POSIX main threads, ``setitimer`` interrupts the running Python code,
    which is important for local agent evaluation because an infinite loop
    must not keep mutating executor state in the background. Other contexts
    use a daemon thread fallback so returning a timeout never waits for the
    timed-out function during executor shutdown.
    ...
    Note:
        The daemon-thread fallback cannot forcefully kill Python code. The
        POSIX path used by Linux/DSW does interrupt it.
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            def run():
                try:
                    outcome.put(("result", func(*args, **kwargs)))
