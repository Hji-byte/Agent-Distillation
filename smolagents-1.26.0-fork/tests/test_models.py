...
class TestTransformersModel:
    def test_init_text_only_skips_vision_loader(self):
        with (
            patch("transformers.AutoModelForCausalLM.from_pretrained") as causal_loader,
            patch("transformers.AutoModelForImageTextToText.from_pretrained") as vision_loader,
        ...
        tokenizer_loader.assert_called_once_with("test-model", trust_remote_code=True)
        assert model.model == causal_loader.return_value
        assert model.tokenizer == tokenizer_loader.return_value
        assert model.flatten_messages_as_text is True
